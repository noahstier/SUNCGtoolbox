...
CreateRoomCameras(void)
{
  ...
  for (int i = 0; i < scene->NNodes(); i++) {
    ...
    room_inds.push_back(i);
  }
  int room_ind = room_inds[0];

  R3SceneNode *room_node = scene->Node(room_ind);
  R2Grid viewpoint_mask;
  ComputeViewpointMask(room_node, viewpoint_mask);

  std::vector<R3Camera> best_cameras;
  std::make_heap(best_cameras.begin(), best_cameras.end(), [](const R3Camera& a, const R3Camera& b) {
    return a.Value() < b.Value();
  });
  unsigned nbest_cameras = 5;

  std::ofstream bbox_file("bbox");
  bbox_file <<
    room_node->BBox().XMin() << " " <<
    room_node->BBox().YMin() << " " <<
    room_node->BBox().ZMin() << " " <<
  ...
  for (int j = 0; j < nangles; j++) {
    ...
    for (RNScalar z = room_bbox.ZMin(); z < room_bbox.ZMax(); z += position_sampling) {
      for (RNScalar x = room_bbox.XMin(); x < room_bbox.XMax(); x += position_sampling) {
        ...
        if (best_cameras.size() < nbest_cameras || camera.Value() > best_cameras[0].Value())
        {
          best_cameras.push_back(camera);
          std::push_heap(best_cameras.begin(), best_cameras.end(), [](const R3Camera& a, const R3Camera& b) {
              return a.Value() > b.Value();
          });

          if (best_cameras.size() > nbest_cameras)
          {
              std::pop_heap(best_cameras.begin(), best_cameras.end(), [](const R3Camera& a, const R3Camera& b) {
                  return a.Value() > b.Value();
              });
              best_cameras.pop_back();
          }
        }
      }
    }
  ...
  for (unsigned i = 0; i < best_cameras.size(); ++i)
  {
    char name[1024];
    sprintf(name, "%s_%d", room_node->Name(), i);
    Camera *camera = new Camera(best_cameras[i], name);
    cameras.Insert(camera);
    camera_count++;
    if (print_debug)
    {
